...
import { AGENT_ID, createAnalyticsStack, evolutionSnapshot } from './create-analytics-stack';
...
async function main(): Promise<void> {
  ...
  app.post('/evolution/extract', async (c) => {
    let body: unknown;
    try {
    ...
    await stack.evolution.extractor().onExtracted(body);
    return c.json(await evolutionSnapshot(stack));
  });
  ...
  console.log(
    `local-self-improvement: ${stack.model} workspace=${stack.workspaceDir} store=${stack.storeDir}`,
  );
  const httpServer = serve({ fetch: app.fetch, port }, () => {
    console.log(`listening on http://localhost:${port}`);
    console.log('  GET  /health');
    console.log('  GET  /evolution');
    console.log('  POST /evolution/extract');
