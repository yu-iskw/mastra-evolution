...
export function inspectWorkspace(workspace: unknown): WorkspaceBind {
  ...
  return {
    basePath,
    storeDirectory: layout.storeDirectory,
    curatedSkillsDirectory: path.join(path.resolve(basePath), firstSkillsSegment(workspace)),
    learnedSkillsDirectory: layout.learnedSkillsDirectory,
    readOnly,
  };
}
